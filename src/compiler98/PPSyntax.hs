...
ppDecl d p (DeclConstrs pos did cs) =
        pp ("-- data/dataprim [(field,selector)] = "
            ++ pStd p did ++ " ["
	    ++ mixComma (map ( \ (pos,field,sel) -> "(" ++ pStd p field ++ "," ++ pStd p sel ++ ")")  cs) ++ "]")
ppDecl d p (DeclClass pos ctxs cls arg decls) =
        pp ("class "++strContexts d p ctxs++" "++(pStd p) cls++" "++ 't':show arg++" ") `cl` ppWhere d p decls
ppDecl d p (DeclInstance pos cxs tycls insts valdefs) =
        pp ("instance " ++ strContexts d p cxs ++ " " ++ (pStd p) tycls ++ " " ++ strInst d p insts)
         `cl` ppWhere d p valdefs
...
ppFun d p (Fun pats gds w) =
   ppSpaceSep (map (ppPat d p) pats) `cl` ppVert (map (ppGdPat "=" d p) gds) `cl` ppWhere d p w
...
ppStmt d p (StmtLet ds) =  (pp ("let {") `cl` nest (ppDecls d p ds)) `nl` pp "}"
...
ppExp d p (ExpLambda    pos pats e) =
        pp (ppPos d pos ++ "(\\ ") `cl` ppSpaceSep (map (ppPat d p) pats) `cl` pp " -> " `cl` 
                ppExp d p e `cl` pp ")"
...
ppExp d p (ExpType      pos e cxs t) =
        pp (ppPos d pos) `cl` ppExp d p e `cl` pp ("::" ++ strContexts d p cxs ++ strType d p t)
ppExp d p (ExpRecord exp fields) =
        ppExp d p exp `cl` pp " {" `cl` ppSemiSep (map (ppField d p) fields) `cl` pp "}"
ppExp d p (ExpApplication pos (f:xs)) =
        pp (ppPos d pos ++ "(") `cl` ppExp d p f `cl` pp " " `cl` ppSpaceSep (map (ppExp d p) xs) `cl` pp ")"
ppExp d p (ExpApplication pos x) =
        pp (ppPos d pos ++ "{- ExpAp -}(") `cl` ppSpaceSep (map (ppExp d p) x) `cl` pp ")"
ppExp d p (ExpInfixList         pos xs) =
        pp (ppPos d pos ++ "{-IL-}(") `cl` ppSpaceSep (map (ppExp d p) xs) `cl` pp ")"
ppExp d p (ExpConOp                pos id) = pp (ppPos d pos ++ "{cop `" ++ (pStd p) id ++ "`}")
ppExp d p (ExpVarOp                pos id) = pp (ppPos d pos ++ "{vop `" ++ (pStd p) id ++ "`}")
...
ppExp d p (ExpList      pos es) =
        pp (ppPos d pos ++ "[") `cl` ppCommaSep (map (ppExp d p) es) `cl` pp "]"
ppExp d p (Exp2  pos id1 id2) = pp (ppPos d pos ++ (pStd p) id1 ++ '.':(pStd p) id2)
