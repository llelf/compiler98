...
This feature is unsupported for this compiler.
\begin{code}
newtype Observer = O ()
defaultObservers label fn = error "observers is not defined for this compiler"
defaultFnObservers label fn a =  error "observers is not defined for this compiler"
\end{code}
...
badEvents :: [Event]
badEvents = [Event {portId = error "Bad Event Stream"}]
...
eventsToCDS pairs = getChild 0 0
   where
     res i = (Array.!) out_arr i

     bnds = (0, length pairs)

     mid_arr :: Array.Array Int [(Int,CDS)]
     mid_arr = Array.accumArray (flip (:)) [] bnds
		[ (pnode,(pport,res node))
	        | (Event node (Parent pnode pport) _) <- pairs
		]

     out_arr = Array.array bnds	-- never uses 0 index
	...
	[ content
        | (pport',content) <- (Array.!) mid_arr pnode
	, pport == pport'
	]
