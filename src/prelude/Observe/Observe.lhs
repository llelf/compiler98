...
#endif
\end{code}
#if !defined(__CLASSIC_HUGS__) && !defined(__NHC__)
\begin{code}
...
instance Observable Ordering	where { observer = observeLit }
...
instance Observable Exception where
	observer exc@(IOException ioe) =  
		sendObservePacket "IOException" (return exc)
	observer (ErrorCall err) =
		sendObservePacket "ErrorCall" $ do
			err <- thunk err
			return (ErrorCall err)
	observer exc = sendObservePacket "<exc>" (return exc)
\end{code}
#endif
...
#else
\begin{code}
ourCatchAll fn context =
	catchAll (fn context) $ \ exc -> do 
		return (sendObservePacket "throw" (do
				    exc' <- thunk exc
				    return (throw exc')) context)
\end{code}
#endif
...
runO program = 
	...
	; catchAllIO
		(program >> return ())
		(\e -> putStrLn ("Top level exception: " ++ show e))
	; closeObserveGlobal
	; return ()
	}
\end{code}
