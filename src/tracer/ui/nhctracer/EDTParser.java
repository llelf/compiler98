...
public class EDTParser {
	...
	switch (t.charAt(0)) {
		...
		switch (t.charAt(0)) {
			...
			trefnr = parseNr();
			irefnr = parseNr();
			result.setTRefNr(trefnr, irefnr);
			...
			if (n instanceof Redex && ((Redex)n).args.size() == 0)
			  ...
			  result = n.spawn(parent, tree, index, trefnr, nodeTable);
		        break;
		    case 'D':
		        refnr = parseNr();
		        result = new CutOffTree(parent, tree, index, refnr);
		    ...
		    case 'C':
		        ...
		        refnr = parseNr();
			srcref = parseSR();
			result = new Case(parent, tree, index, srcref, refnr, caseifgd==0?"if ":caseifgd==1?"case ":"| ");
			EDTNode e = parseEDTNode((EDTStructuredNode)result, tree, index);
			EDTNode ctxt = parseEDTNode((EDTStructuredNode)result, tree, index);
			irefnr = parseNr();
			((Case)result).setArgs(e, ctxt, ctxt.refnr, irefnr);
			nodeTable.setNodeAt(result, refnr);
			break;
		    case 'B':
		        refnr = parseNr();	      
