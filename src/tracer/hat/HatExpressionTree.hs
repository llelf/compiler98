...
-- Entire expression held in memory.
-- Multiple access to any part of the expression
-- will only result in one access to the file.

 HatExpressionTree(HatApplication, HatConstant, HatConstructor, HatIdentifier
                  ,HatSAT_A, HatSAT_B, HatHidden, HatProj, HatCase, HatLambda
                  ,HatInt, HatChar, HatInteger, HatDummy
		  ,HatRational, HatFloat, HatDouble, HatString
                  ,HatIf, HatGuard, HatContainer, HatNone),
 toHatExpressionTree,
 HatLimit,
 toHatLimit,
 fromHatLimit,
...
toHatExpressionTree precision node =
    _toHatExpressionTree precision (toHatNode node)

_toHatExpressionTree :: Int -> HatNode -> HatExpressionTree
_toHatExpressionTree precision node =
    toHatExpression' precision (hatNodeType node)
 where
   toHatExpression' :: Int -> HatNodeType -> HatExpressionTree
   ...
   toHatExpression' precision HatApplNode = 
       ...
       HatApplication node 
		      ...
		      (let r = (hatResult node)
		       in if (isInvalidNode r) then (HatNone node) else
   ...
   toHatExpression' prec HatConstantNode =
       ...
       HatConstant node
		   ...
		   (let r = (hatResult node)
		    in if (isInvalidNode r) then (HatNone node) else
		                                 _toHatExpressionTree newprec r)
   toHatExpression' prec HatSAT_ANode  = HatSAT_A node
                                                  (_toHatExpressionTree (prec-1)
						            (hatProjValue node))
   toHatExpression' prec HatSAT_BNode  = HatSAT_B node
					          (_toHatExpressionTree (prec-1)
						            (hatProjValue node))
   toHatExpression' prec HatSAT_CNode  = _toHatExpressionTree (prec-1)
					                    (hatProjValue node)
   toHatExpression' prec HatHiddenNode = HatHidden node
					          (_toHatExpressionTree (prec-1)
						            (hatParent node))
   toHatExpression' prec HatProjNode   = HatProj node
					         (_toHatExpressionTree (prec-1)
						            (hatParent node))
					         (_toHatExpressionTree (prec-1) 
					                    (hatProjValue node))
...
ppStringExpr precision expr =
  ...
  if (exptype==HatProjNode) then
       (ppStringExpr precision (hatProjValue expr))
  else if (exptype/=HatConstantNode)&&(exptype/=HatApplNode) then
       Nothing
  else if ((hatNodeType appfun)==HatInvalidNode) then
       Nothing
  else let funsym = (prettyPrint 2 False appfun) in
       if funsym==":" then
           let arguments = (hatApplArgs expr) in
           if (length arguments)/=2 then
                Nothing
           else let f = (head arguments) in
                if (hatNodeType f)==HatConstantNode then
                     pp' (hatApplFun f) (head (tail arguments))
                else Nothing
       else if funsym=="[]" then (Just "") else Nothing
 where 
...
pPrint previousnodes precision verbose topInfix expr
  ...
  | otherwise =
      ...
      if (hn `elem` previousnodes) then
           ("a",HatNoInfix,[hn])
      else let r@(s,i,cs)=prettyPrint' (hn:previousnodes) (hatNodeType expr) in
           if (hn `elem` cs) then
  ...
  where
    prettyPrint' pnodes HatApplNode =
        ...
        if (isJust s) then
             if (fromJust s)=="" then
                  ("[]",HatNoInfix,[])
	     else ("\""++(fromJust s)++"\"",HatNoInfix,[])
        else if (precision>0) then
             let (funsym,infixP,cycles) = pPrint pnodes (precision-1)
					         verbose topInfix
					         (hatApplFun expr) in
             if funsym == "<CUT>" then
		    (funsym,infixP,[])
             else let args = (hatApplArgs expr) in
                  if (funsym == "IO")&&((length args)==1) then
	               -- show HIDDEN argument of IO (otherwise only
                       --   IO <HIDDEN> was shown)
		       let f = (head args);
                           p = (hatParent f);
		           arg = if ((hatNodeType f)==HatHiddenNode)
                                          && (isValidNode p) then p else f;
		           (s,_,cycles2) = pPrint pnodes (precision-1)
                                                  verbose infixP arg
		       in
		       (brackets infixP topInfix (spacer funsym s)
                       ,infixP,cycles2)
                  else let pps = map (pPrint pnodes (precision-1)
                                             verbose infixP) args
                       in
		       (brackets infixP topInfix
                                 (foldl (spacer) ""
                                        (swapListInfix infixP funsym
                                                       (map fst3 pps)))
                       ,infixP
                       ,(cycles++(foldl (++) [] (map thrd3 pps))))
        else ("<CUT>",HatNoInfix,[])
    prettyPrint' pnodes HatConstantNode = pPrint pnodes precision verbose
                                                 topInfix (hatApplFun expr)
    prettyPrint' _ HatHiddenNode     = ("<Hidden>",HatNoInfix,[])
    prettyPrint' pnodes HatSAT_ANode = if (verbose) then
				          pPrint pnodes precision verbose
                                                 topInfix (hatProjValue expr)
				       else ("_",HatNoInfix,[])
    ...
    prettyPrint' _ HatIntNode       = (show (hatValueInt expr)
                                      ,HatNoInfix,[])
    prettyPrint' _ HatCharNode      = (show (hatValueChar expr)
                                      ,HatNoInfix,[])
    prettyPrint' _ HatIntegerNode   = (show (hatValueInteger expr)
                                      ,HatNoInfix,[])
    prettyPrint' _ HatRationalNode  = (show (hatValueRational expr)
                                      ,HatNoInfix,[])
    prettyPrint' _ HatFloatNode     = (show (hatValueFloat expr)
                                      ,HatNoInfix,[])
    prettyPrint' _ HatDoubleNode    = (show (hatValueDouble expr)
                                      ,HatNoInfix,[])
    prettyPrint' _ HatCStringNode   = (ppString (hatValueString expr)
                                      ,HatNoInfix,[])
    ...
    prettyPrint' pnodes HatProjNode = pPrint pnodes (precision-1) verbose
                                             topInfix (hatProjValue expr)
