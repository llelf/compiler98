...
checkParameters l ('-':r) = checkParameters' l r
    where 
    checkParameters' _ [] = 0
    checkParameters' l (c:r) = if (c `elem` l) then (checkParameters' l r)
                                               else 2
...
doCommand cmd s hatfile@(file,_) state@(lastObserved,more,equationsPerPage,
	...
	putStrLn ("Equations per page is currently set to "
                  ++(show equationsPerPage)++".") >> interactive hatfile state
...
interactiveHelp =
  do
   ...
   putStrLn " command           abbrev.     meaning"
   putStrLn "---------------------------------------------------------------------------"
   ...
   putStrLn ""
   putStrLn "Press <RETURN> for query syntax, any other key to quit help."
   s <- getLine
   if (null s) then queryHelp else return ()

queryHelp = do
   putStrLn "---------------------------------------------------------------------------"
   putStrLn "An 'observe' query requires a function identifier, and can optionally"
   putStrLn "pattern-match on: specific arguments to the function;, a specific"
   putStrLn "result; a specific caller function.  The full query syntax is:"
   putStrLn "    <identifier> [<pattern>]*  ['=' <pattern>]?  ['in' <identifier>]?"
   putStrLn "where"
   putStrLn "    <pattern> = _                                   wildcard"
   putStrLn "              | ''' <char> '''                      character"
   putStrLn "              | '\"' <string> '\"'                    string"
   putStrLn "              | <num>                               number"
   putStrLn "              | '[' <pattern> [',' <pattern>]* ']'  literal list"
   putStrLn "              | <Constr>                            nullary constructor"
   putStrLn "              | '(' <Constr> [<pattern>]* ')'       constructor application"
   putStrLn "Note: Currently INFIX operators are not supported in patterns."
...
showHelp = do
   putStrLn "Usage:   hat-observe [-v] [-r] [-xu] identifier [in topidentifier] file-name"
   ...
   putStrLn "       r: recursive mode.  Omit recursive function applications."
