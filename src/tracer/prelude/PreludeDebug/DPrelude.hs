...
data Value = Evaluating | Evaluated | Closure
...
cCheckEvaluation primitive 1 :: E a -> Value
...
fun11 nm rf sr t = 
    R (\t a -> R (\t b -> R (\t c -> R (\t d -> R (\t e -> R (\t f -> R (\t g -> R (\t h -> R (\t i -> R (\t j -> R (\t k -> let (R r rt) = {-enter nm t (-}rf t a b c d e f g h i j{-)-} in R r (Sat t rt)) t) t) t) t) t) t) t) t) t) t) (Nm t nm sr)
fun12 nm rf sr t = 
    R (\t a -> R (\t b -> R (\t c -> R (\t d -> R (\t e -> R (\t f -> R (\t g -> R (\t h -> R (\t i -> R (\t j -> R (\t k -> R (\t l -> let (R r rt) = {-enter nm t (-}rf t a b c d e f g h i j{-)-} in R r (Sat t rt)) t) t) t) t) t) t) t) t) t) t) t) (Nm t nm sr)
