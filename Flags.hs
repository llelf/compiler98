...
allOpts =
    ...
    , Opt "i" "hidst" (StrFlag "." $ \ f x -> f{sTypeDst=x}) "destination path for generated .hi file"
...
--                                                    "from file name and don't create profiling information for this module")
    , Opt "" "zap" (bt $ \f x -> f{sZap=x}) "Zap unused args/stack positions"

    , OptGroup "Profiling" True
    , Opt "p" "profile" (bf $ \f x -> f{sProfile=x}) "Generate code for heap profiling"
    , Opt "z" "tprof" (bf $ \f x -> f{sTprof=x}) "Generate code for time profiling"
   
    , OptGroup "Compliance Options" True
    , Opt ""  "ansiC" (bt $ \f x -> f{sAnsiC=x}) "Generate bytecode via ANSI-C"
    , Opt ""  "nplusk" (bt $ \f x -> f{sNplusK=x}) "Allow (n+k) patterns"
